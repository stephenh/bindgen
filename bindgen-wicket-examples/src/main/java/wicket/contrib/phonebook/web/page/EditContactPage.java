...
import org.bindgen.BindKeyword;
...
public class EditContactPage extends BasePage
{
    ...
    public EditContactPage(Page backPage, IModel< ? > contactModel)
    {
        ...
        form.add(newRequiredTextField("firstname", new BindingModel<String>(BindKeyword.bind(this)
                .contact().firstname()), 32));
        form.add(newRequiredTextField("lastname", new BindingModel<String>(BindKeyword.bind(this)
                .contact().lastname()), 32));
        form.add(newRequiredTextField("phone", new BindingModel<String>(BindKeyword.bind(this)
                .contact().phone()), 16));
        form.add(new TextField<String>("email", new BindingModel<String>(BindKeyword.bind(this)
                .contact().email())).add(StringValidator.maximumLength(128)).add(
                EmailAddressValidator.getInstance()));
        form.add(new CancelButton());
        form.add(new SaveButton());
    }

    private RequiredTextField<String> newRequiredTextField(String id, IModel<String> model,
            int maxLenght)
    {
        RequiredTextField<String> textField = new RequiredTextField<String>(id, model);
